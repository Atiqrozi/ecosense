...
//    }
    ...
    private fun analyzeImage() {
        mainViewModel.currentImageUri.observe(this) { uri ->
            val imageFile = uri?.let { uriToFile(it, this)?.reduceFileImage() }
            ...
            imageClassifierHelper = ImageClassifierHelper(context = this,
                classifierListener = object : ImageClassifierHelper.ClassifierListener {
                    ...
                    override fun onResult(result: List<Classifications>) {
                        result.firstOrNull()?.let { classification ->
                            // Jika terdapat setidaknya satu hasil klasifikasi
                            if (classification.categories.size == 1) {
                                val category = classification.categories.first()
                                val displayResult = "${category.label}: ${category.score * 100}%"
                                if (uri != null) {
                                    moveToResult(uri, displayResult)
                                }
                            } else {
                                showToast("Unexpected number of classifications found")
                            }
                        } ?: run {
